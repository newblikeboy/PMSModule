...
const pendingFlows = new Map(); // flowId -> { userId, createdAt }
...
function registerFlow(userId) {
  ...
  pendingFlows.set(id, { userId: String(userId), createdAt: Date.now() });
  setTimeout(() => pendingFlows.delete(id), FLOW_TTL_MS).unref?.();
  return id;
}

function consumeFlow(flowId) {
  if (!flowId) return null;
  const entry = pendingFlows.get(flowId);
  if (entry) pendingFlows.delete(flowId);
  return entry;
}
...
async function buildLoginUrlForUserId(userId) {
  ...
  const flowId = registerFlow(user._id);
